// src/tabs/dashboard/view.rs — all UI rendering for the Dashboard tab
...
pub fn render(tab: &DashboardTab) -> Element<'_, Message> {
    ...
    let services = row![
        service_card("Apache", "HTTP Server", tab.apache_running, GREEN,
            Message::StartApache, Message::StopApache, Message::RestartApache),
        service_card("MySQL",  "Database",    tab.mysql_running,  BLUE,
            Message::StartMySQL,  Message::StopMySQL,  Message::RestartMySQL),
        php_card(tab),
    ]
    .spacing(12);
...
fn service_card<'a>(
    ...
    let btn_row = row![
        button(text("Start").size(13).width(Length::Fill).center())
            .on_press(start).padding(Padding::from([7, 0])).width(Length::FillPortion(1))
            .style(btn_style(BTN_SUCCESS)),
        button(text("Stop").size(13).width(Length::Fill).center())
            .on_press(stop).padding(Padding::from([7, 0])).width(Length::FillPortion(1))
            .style(btn_style(BTN_DANGER)),
        button(text("Restart").size(13).width(Length::Fill).center())
            .on_press(restart).padding(Padding::from([7, 0])).width(Length::FillPortion(1))
...
fn php_card(tab: &DashboardTab) -> Element<'_, Message> {
    ...
    let running_dot = container(
        row![status_dot(PURPLE), Space::with_width(5), text(version_text).size(11).color(PURPLE)]
            .align_y(Alignment::Center),
    )
    .padding(Padding::from([4, 9]))
    ...
    let picker: Element<Message> = if !tab.php_versions.is_empty() {
        ...
        container(text("No PHP detected").size(13).color(TEXT_MUTED))
            .padding(Padding::from([9, 12])).width(Length::Fill)
            .style(|_: &iced::Theme| container::Style {
        ...
        .style(|_, status| match status {
            iced::widget::button::Status::Hovered | iced::widget::button::Status::Pressed =>
                iced::widget::button::Style {
                    ...
                    border: Border { color: PURPLE_BDR, width: 1.0, radius: 7.0.into() }, ..Default::default()
                },
            _ => iced::widget::button::Style {
                background: Some(PURPLE_BG.into()), text_color: PURPLE,
                border: Border { radius: 7.0.into(), ..Default::default() }, ..Default::default()
            },
        });

    container(column![
        top, Space::with_height(14),
        text("PHP Engine").size(19).color(TEXT_PRIMARY), Space::with_height(3),
        text("Version Switcher").size(12).color(TEXT_MUTED), Space::with_height(16),
        thin_line(), Space::with_height(14),
        text("Active Version").size(11).color(TEXT_MUTED), Space::with_height(6),
        picker, Space::with_height(8),
        php_info_btn,
    ].spacing(0))
    .padding(Padding::from([18, 18]))
    .width(Length::FillPortion(1))
...
fn sep_vertical() -> Element<'static, Message> {
    container(Space::with_width(1)).width(1).height(12)
        .style(|_: &iced::Theme| container::Style { background: Some(BORDER_MED.into()), ..Default::default() })
        .into()
}
