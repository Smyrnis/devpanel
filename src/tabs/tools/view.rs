...
fn php_panel(tab: &ToolsTab) -> Element<'_, Message> {
    ...
    let header = row![
        ...
        button(text(scan_lbl).size(12).color(TEAL))
            .on_press_maybe(if tab.scanning { None } else { Some(Message::TOOLS_ScanPhp) })
...
fn apache_mods_panel(tab: &ToolsTab) -> Element<'_, Message> {
    ...
    let header = row![
        ...
        button(text(scan_lbl).size(12).color(TEAL))
            .on_press_maybe(if tab.mods_scanning { None } else { Some(Message::TOOLS_ScanApacheMods) })
            .padding(Padding::from([7, 14]))
            .style(|_, status| match status {
                iced::widget::button::Status::Hovered | iced::widget::button::Status::Pressed =>
                    iced::widget::button::Style { background: Some(TEAL_HOVER.into()), text_color: TEAL, border: Border { radius: 8.0.into(), ..Default::default() }, ..Default::default() },
    ...
    container(column![
        ...
        filter_row, Space::with_height(14),
        thin_line(), Space::with_height(10),
        body, Space::with_height(16),
        container(row![
...
fn error_suggestion_panel(tab: &ToolsTab) -> Element<'_, Message> {
    let php_version = tab.install_log.iter().rev()
...
fn db_btn<'a>(title: &'a str, subtitle: &'a str, accent: Color, bg: Color, bg_hover: Color, _border: Color, msg: Message) -> Element<'a, Message> {
    button(row![
        container(Space::with_width(4)).width(4).height(28)
            .style(move |_: &iced::Theme| container::Style { background: Some(accent.into()), border: Border { radius: 2.0.into(), ..Default::default() }, ..Default::default() }),
        Space::with_width(12),
