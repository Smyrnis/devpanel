...
fn list_view(tab: &VHostsTab) -> Element<'_, Message> {
    ...
    let path_bar = container(row![
        ...
        icon_btn("Edit Config", BLUE, BLUE_BG, BLUE_HOVER, BLUE_BORDER,
            Some(Message::VHosts(VHostsMessage::OpenConfigEditor))),
        Space::with_width(8),
        icon_btn("Open File", BLUE, BLUE_BG, BLUE_HOVER, BLUE_BORDER,
        ...
        icon_btn(if tab.scanning { "Scanning…" } else { "Reload" }, TEAL, TEAL_BG, TEAL_HOVER, TEAL_BORDER,
            if tab.scanning { None } else { Some(Message::VHosts(VHostsMessage::Scan)) }),
        Space::with_width(8),
        icon_btn("+ Add VHost", GREEN, GREEN_BG, GREEN_HOVER, GREEN_BG,
    ...
    let body: Element<Message> = if tab.vhosts.is_empty() && !tab.scanning {
        ...
        column(tab.vhosts.iter().map(|v| vhost_row(tab, v)).collect::<Vec<_>>()).spacing(8).into()
    };
...
fn config_editor_view(tab: &VHostsTab) -> Element<'_, Message> {
    let header = row![
        ...
        icon_btn("← Back", TEAL, TEAL_BG, TEAL_HOVER, TEAL_BORDER,
            Some(Message::VHosts(VHostsMessage::CloseConfigEditor))),
        Space::with_width(8),
        icon_btn(if tab.config_loading { "Saving…" } else { "Save" }, GREEN, GREEN_BG, GREEN_HOVER, GREEN_BG,
            if tab.config_loading { None } else { Some(Message::VHosts(VHostsMessage::SaveConfigFile)) }),
    ].align_y(Alignment::Center);
    ...
    let editor = text_editor(&tab.config_content)
        ...
        .height(Length::Fill).padding(Padding::from([12, 14]));

    let editor_container = container(editor).width(Length::Fill).height(Length::Fill)
    ...
    column![
        ...
        container(column![header, Space::with_height(8), dirty_badge].spacing(0)
            .padding(Padding::from([0, 24]))).width(Length::Fill),
...
fn vhost_row<'a>(tab: &'a VHostsTab, vh: &'a VHostEntry) -> Element<'a, Message> {
    ...
    let name_row = row![
        ...
        container(text("active").size(10).color(GREEN)).padding(Padding::from([3, 8]))
            .style(|_: &iced::Theme| container::Style { background: Some(GREEN_BG.into()),
                border: Border { radius: 20.0.into(), ..Default::default() }, ..Default::default() }),
    ].align_y(Alignment::Center);
    ...
    let php_badge: Element<Message> = match &vh.php_version {
        Some(ver) => container(row![
            text("PHP").size(9).color(PURPLE), Space::with_width(4),
            text(ver.as_str()).size(10).color(PURPLE),
        ].align_y(Alignment::Center)).padding(Padding::from([3, 8]))
        .style(|_: &iced::Theme| container::Style { background: Some(PURPLE_BG.into()),
            border: Border { color: PURPLE_BDR, width: 1.0, radius: 20.0.into() }, ..Default::default() }).into(),
        None => container(text("global PHP").size(9).color(TEXT_MUTED)).padding(Padding::from([3, 8]))
    ...
    let del_btn: Element<Message> = if is_confirming {
        row![
            small_btn("Confirm Delete", RED, RED_BG, RED_HOVER, RED_BG,
                Some(Message::VHosts(VHostsMessage::DeleteConfirm(idx)))),
            Space::with_width(6),
            small_btn("Cancel", TEXT_MUTED, BG_SURFACE, BG_HOVER, BORDER_SUBTLE,
                Some(Message::VHosts(VHostsMessage::DeleteCancel))),
        ].align_y(Alignment::Center).into()
    } else {
        small_btn("Delete", RED, RED_BG, RED_HOVER, RED_BG,
...
fn inline_edit_widget<'a>(tab: &'a VHostsTab, _idx: usize) -> Element<'a, Message> {
    ...
    let submit_btn = button(text("Save Changes").size(13).color(if can_save { GREEN } else { TEXT_MUTED }))
        ...
        .style(move |_, status| match status {
            iced::widget::button::Status::Hovered if can_save =>
                iced::widget::button::Style { background: Some(GREEN_HOVER.into()), text_color: GREEN,
    ...
    let submit_el: Element<Message> = if can_save {
        submit_btn.on_press(Message::VHosts(VHostsMessage::SaveEdit)).into()
    } else { submit_btn.into() };
    ...
    container(column![
        row![
            text("Editing VirtualHost").size(13).color(BLUE), Space::with_width(Length::Fill),
            small_btn("Cancel", TEXT_MUTED, BG_SURFACE, BG_HOVER, BORDER_SUBTLE,
        ...
        row![
            column![
                text("ServerName").size(11).color(TEXT_MUTED), Space::with_height(5),
            ...
            Space::with_width(14),
            column![
                text("DocumentRoot").size(11).color(TEXT_MUTED), Space::with_height(5),
    ...
    .style(|_: &iced::Theme| container::Style { background: Some(BG_CARD.into()),
        border: Border { color: BLUE_BORDER, width: 1.5, radius: 10.0.into() }, ..Default::default() }).into()
}

fn add_form_widget(tab: &VHostsTab) -> Element<'_, Message> {
    let is_edit = matches!(tab.form.mode, FormMode::Edit(_));
    let can_save = !tab.form.server_name.trim().is_empty() && !tab.form.document_root.trim().is_empty();
    let save_msg = if is_edit { Message::VHosts(VHostsMessage::SaveEdit) } else { Message::VHosts(VHostsMessage::Create) };
    ...
    container(column![
        ...
        column![
            text("ServerName  (e.g. myproject.local)").size(11).color(TEXT_MUTED), Space::with_height(5),
        ...
        column![
            row![
                text("PHP Version").size(11).color(TEXT_MUTED), Space::with_width(6),
                text("(optional — pins this vhost to a specific PHP via SetHandler)").size(10).color(TEXT_MUTED),
            ].align_y(Alignment::Center),
    ...
    F: Fn(String) -> Message + 'a,
{
    ...
    let el = pick_list(options, Some(selected), move |s: String| on_select(s))
        .padding(Padding::from([8, 12])).width(Length::Fixed(220.0))
        .style(|_theme, status| {
            use iced::widget::pick_list;
            let open = matches!(status, pick_list::Status::Opened);
            pick_list::Style {
                text_color: PURPLE, placeholder_color: TEXT_MUTED, handle_color: PURPLE,
                background: iced::Background::Color(if open { PURPLE_BG2 } else { PURPLE_BG }),
                border: Border { color: if open { PURPLE } else { PURPLE_BDR }, width: 1.0, radius: 8.0.into() },
            }
        });

    if available.is_empty() {
        column![
            el, Space::with_height(3),
            text("No mod_phpX.Y enabled — only global PHP available").size(10).color(TEXT_MUTED),
        ].spacing(0).into()
    } else { el.into() }
}
...
fn small_btn<'a>(label: &'a str, color: Color, bg: Color, bg_hover: Color, border: Color, on_press: Option<Message>) -> Element<'a, Message> {
    let b = button(text(label).size(12).color(color)).padding(Padding::from([6, 12]))
        .style(move |_, status| match status {
            iced::widget::button::Status::Hovered | iced::widget::button::Status::Pressed =>
                iced::widget::button::Style { background: Some(bg_hover.into()), text_color: color,
                    border: Border { color: border, width: 1.0, radius: 7.0.into() }, ..Default::default() },
...
fn icon_btn<'a>(label: &'a str, color: Color, bg: Color, bg_hover: Color, border: Color, on_press: Option<Message>) -> Element<'a, Message> {
    let b = button(text(label).size(12).color(color)).padding(Padding::from([7, 14]))
        .style(move |_, status| match status {
            iced::widget::button::Status::Hovered | iced::widget::button::Status::Pressed =>
                iced::widget::button::Style { background: Some(bg_hover.into()), text_color: color,
                    border: Border { color: border, width: 1.0, radius: 8.0.into() }, ..Default::default() },
            _ => iced::widget::button::Style { background: Some(bg.into()), text_color: color,
                border: Border { color: border, width: 1.0, radius: 8.0.into() }, ..Default::default() },
        });
    if let Some(msg) = on_press { b.on_press(msg).into() } else { b.into() }
