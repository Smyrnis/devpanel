// src/tabs/ssh_keys/view.rs — all UI rendering for the SSH Keys tab

use super::{KeyType, StatusKind, SshKeysTab};
use crate::theme::*;
use crate::Message;
...
fn generate_panel(tab: &SshKeysTab) -> Element<'_, Message> {
    let type_row = row![
        radio("Ed25519",  KeyType::Ed25519, Some(tab.key_type), |_| Message::SSH_KeyTypeChanged(KeyType::Ed25519)),
        Space::with_width(14),
        radio("RSA 4096", KeyType::Rsa4096, Some(tab.key_type), |_| Message::SSH_KeyTypeChanged(KeyType::Rsa4096)),
        Space::with_width(14),
        radio("ECDSA",    KeyType::Ecdsa,   Some(tab.key_type), |_| Message::SSH_KeyTypeChanged(KeyType::Ecdsa)),
    ].align_y(Alignment::Center);

    let pass_row = row![
        text_input(
            if tab.show_passphrase { "passphrase" } else { "(hidden)" },
            &tab.passphrase,
        )
        .on_input(Message::SSH_PassphraseChanged)
        .secure(!tab.show_passphrase)
        .padding(10).size(13).width(Length::Fill),
        button(text(if tab.show_passphrase { "Hide" } else { "Show" }).size(12))
            .on_press(Message::SSH_TogglePassphrase(!tab.show_passphrase))
    ...
    container(column![
        ...
        text_input("user@example.com", &tab.email)
            .on_input(Message::SSH_EmailChanged).padding(10).size(13),
        Space::with_height(14),
        lbl("Key filename"), Space::with_height(5),
        text_input("id_ed25519", &tab.key_name)
            .on_input(Message::SSH_KeyNameChanged).padding(10).size(13),
        Space::with_height(14),
        lbl("Key type"), Space::with_height(8),
        type_row,
        Space::with_height(14),
        lbl("Passphrase (optional)"), Space::with_height(5),
        pass_row,
        Space::with_height(20),
        divider,
        Space::with_height(16),
        ...
        Space::with_height(8),
        row![
            button(text("Add Existing").size(12)).on_press(Message::SSH_AddExisting).padding(Padding::from([8, 14])).style(ghost_style()),
            button(text("Open ~/.ssh").size(12)).on_press(Message::SSH_OpenDir).padding(Padding::from([8, 14])).style(ghost_style()),
            button(text("Refresh").size(12)).on_press(Message::SSH_ListKeys).padding(Padding::from([8, 14])).style(ghost_style()),
        ].spacing(8),
