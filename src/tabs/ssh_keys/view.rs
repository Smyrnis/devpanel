    ...
    scrollable(
        column![
            column![
                ...
                Space::with_height(4),
                text("Generate and manage SSH keys for this machine").size(13).color(TEXT_MUTED),
            ].spacing(0),
            Space::with_height(22),
            row![generate_panel(tab), Space::with_width(14), keys_panel(tab)].align_y(Alignment::Start),
...
fn generate_panel(tab: &SshKeysTab) -> Element<'_, Message> {
    let type_row = row![
        radio("Ed25519", KeyType::Ed25519, Some(tab.key_type), |_| Message::SshKeys(SshKeysMessage::KeyTypeChanged(KeyType::Ed25519))),
        Space::with_width(14),
        radio("RSA 4096", KeyType::Rsa4096, Some(tab.key_type), |_| Message::SshKeys(SshKeysMessage::KeyTypeChanged(KeyType::Rsa4096))),
        Space::with_width(14),
        radio("ECDSA", KeyType::Ecdsa, Some(tab.key_type), |_| Message::SshKeys(SshKeysMessage::KeyTypeChanged(KeyType::Ecdsa))),
    ].align_y(Alignment::Center);

    let pass_row = row![
        text_input(if tab.show_passphrase { "passphrase" } else { "(hidden)" }, &tab.passphrase)
            .on_input(|v| Message::SshKeys(SshKeysMessage::PassphraseChanged(v)))
            .secure(!tab.show_passphrase).padding(10).size(13).width(Length::Fill),
        button(text(if tab.show_passphrase { "Hide" } else { "Show" }).size(12))
            .on_press(Message::SshKeys(SshKeysMessage::TogglePassphrase(!tab.show_passphrase)))
            .padding(Padding::from([10, 14])).style(ghost_style()),
    ...
    container(column![
        ...
        lbl("Email address"), Space::with_height(5),
        text_input("user@example.com", &tab.email)
            .on_input(|v| Message::SshKeys(SshKeysMessage::EmailChanged(v))).padding(10).size(13),
        Space::with_height(14),
        lbl("Key filename"), Space::with_height(5),
        text_input("id_ed25519", &tab.key_name)
            .on_input(|v| Message::SshKeys(SshKeysMessage::KeyNameChanged(v))).padding(10).size(13),
        Space::with_height(14),
        lbl("Key type"), Space::with_height(8), type_row,
        Space::with_height(14),
        lbl("Passphrase (optional)"), Space::with_height(5), pass_row,
        Space::with_height(20), divider, Space::with_height(16),
        button(text("Generate Key").size(13))
            .on_press(Message::SshKeys(SshKeysMessage::GenerateKey))
            .padding(Padding::from([10, 22])).style(btn_style(ACCENT)),
        Space::with_height(16),
        text("Quick Actions").size(11).color(TEXT_MUTED), Space::with_height(8),
        row![
            button(text("Add Existing").size(12))
                .on_press(Message::SshKeys(SshKeysMessage::AddExisting))
                .padding(Padding::from([8, 14])).style(ghost_style()),
    ...
    ].spacing(0).padding(Padding::from([22, 22]))).width(Length::FillPortion(3)).style(card_style()).into()
}
...
fn keys_panel(tab: &SshKeysTab) -> Element<'_, Message> {
    let entries: Vec<Element<Message>> = if tab.keys_list.is_empty() {
        vec![container(text("No keys found. Click Refresh.").size(13).color(TEXT_MUTED))
            .padding(Padding::from([20, 16])).into()]
    } else {
        tab.keys_list.iter().map(|k| {
            ...
            .padding(Padding::from([11, 14])).width(Length::Fill)
            .style(|_: &iced::Theme| container::Style {
    ...
    container(column![
        row![
            text("SSH Keys").size(14).color(TEXT_SECONDARY).width(Length::Fill),
            container(text(format!("{}", tab.keys_list.len())).size(11).color(TEXT_MUTED))
                .padding(Padding::from([3, 8]))
                .style(|_: &iced::Theme| container::Style {
                    background: Some(BG_SURFACE.into()),
    ...
    ].spacing(0).padding(Padding::from([22, 22]))).width(Length::FillPortion(2)).style(card_style()).into()
}
...
fn status_bar(tab: &SshKeysTab) -> Element<'_, Message> {
    ...
    let (color, border_color, icon) = match tab.status_kind {
        StatusKind::Success => (GREEN, Color { r: 0.070, g: 0.210, b: 0.110, a: 1.0 }, "+"),
        StatusKind::Error => (RED, Color { r: 0.300, g: 0.090, b: 0.080, a: 1.0 }, "x"),
        StatusKind::Info => (BLUE, Color { r: 0.080, g: 0.140, b: 0.260, a: 1.0 }, "i"),
...
fn ghost_style() -> impl Fn(&iced::Theme, iced::widget::button::Status) -> iced::widget::button::Style {
    |_, status| match status {
        ...
        _ => iced::widget::button::Style { background: Some(BG_CARD.into()), text_color: TEXT_SECONDARY,
