...
const TEAL_BORDER: Color = Color { r: 0.100, g: 0.200, b: 0.118, a: 1.0 };
...
fn provider_color(p: &Provider) -> Color {
    match p { Provider::GitHub => TEAL, Provider::Bitbucket => BLUE }
}
fn provider_bg(p: &Provider) -> Color {
    match p { Provider::GitHub => TEAL_BG, Provider::Bitbucket => BLUE_BG }
}
fn provider_border(p: &Provider) -> Color {
    match p { Provider::GitHub => TEAL_BORDER, Provider::Bitbucket => BLUE_BORDER }
}
...
pub fn render(tab: &ReposTab) -> Element<'_, Message> {
    ...
    let status_bar = container(
        row![
            ...
            icon_btn(if tab.fetching { "Fetching…" } else { "Fetch Repos" }, TEAL, TEAL_BG, TEAL_HOVER, TEAL_BORDER,
                if tab.fetching { None } else { Some(Message::Repos(ReposMessage::Fetch)) }),
            Space::with_width(8),
            icon_btn("Check SSH", TEXT_SECONDARY, BG_HOVER, BG_ELEVATED, BORDER_SUBTLE,
                Some(Message::Repos(ReposMessage::CheckSsh))),
            Space::with_width(8),
            icon_btn("Open Projects", TEXT_SECONDARY, BG_HOVER, BG_ELEVATED, BORDER_SUBTLE,
        ...
        container(
            row![
                ...
                text_input("Search repos…", &tab.search_query)
                    ...
                    .size(12).padding(Padding::from([6, 10])).width(Length::Fill),
            ].align_y(Alignment::Center),
        ).padding(Padding::from([10, 16])).width(Length::Fill).style(surface_style()).into()
    } else {
    ...
    let toast: Element<Message> = match &tab.status_msg {
        Some((ok, msg)) => {
            ...
            container(
                row![dot(color), Space::with_width(8), text(msg.as_str()).size(12).color(TEXT_SECONDARY)]
                    .align_y(Alignment::Center),
    ...
    let body: Element<Message> = if tab.remote_repos.is_empty() && !tab.fetching {
        ...
        container(column![
            text("Fetching repositories via SSH…").size(14).color(TEXT_MUTED),
            Space::with_height(8),
            text("This may take a moment").size(12).color(TEXT_MUTED),
        ].align_x(Alignment::Center))
        .width(Length::Fill).padding(Padding::from([48, 0])).center_x(Length::Fill).into()
    } else {
        let q = tab.search_query.to_lowercase();
        let filtered: Vec<&RemoteRepo> = tab.remote_repos.iter().filter(|r| {
    ...
    scrollable(
        column![
            ...
            status_bar, Space::with_height(10),
            filter_bar,
...
fn ssh_pill<'a>(label: &'a str, status: &'a SshStatus) -> Element<'a, Message> {
    ...
    container(
        row![
            dot(dot_color), Space::with_width(6),
            text(label).size(11).color(TEXT_SECONDARY), Space::with_width(4),
...
fn empty_state<'a>() -> Element<'a, Message> {
    let github_hint = container(column![
        row![text("GitHub").size(12).color(TEAL), Space::with_width(8), text("ssh -T git@github.com").size(11).color(TEXT_MUTED)].align_y(Alignment::Center),
        Space::with_height(4),
        text("Ensure your SSH key is added at github.com/settings/keys").size(11).color(TEXT_MUTED),
    ].spacing(0)).padding(Padding::from([12, 14])).width(Length::Fill)
    .style(|_: &iced::Theme| container::Style {
        background: Some(TEAL_BG.into()),
        border: Border { color: TEAL_BORDER, width: 1.0, radius: 8.0.into() },
        ..Default::default()
    });

    let bitbucket_hint = container(column![
        row![text("Bitbucket").size(12).color(BLUE), Space::with_width(8), text("ssh -T git@bitbucket.org").size(11).color(TEXT_MUTED)].align_y(Alignment::Center),
        Space::with_height(4),
        text("Ensure your SSH key is added at bitbucket.org/account/settings/ssh-keys/").size(11).color(TEXT_MUTED),
    ].spacing(0)).padding(Padding::from([12, 14])).width(Length::Fill)
    .style(|_: &iced::Theme| container::Style {
...
fn repo_card<'a>(repo: &'a RemoteRepo) -> Element<'a, Message> {
    ...
    let provider_badge = container(text(repo.provider.label()).size(10).color(p_color))
        ...
        .style(move |_: &iced::Theme| container::Style {
            ...
            border: Border { color: p_border, width: 1.0, radius: 20.0.into() },
            ..Default::default()
        });
    ...
    let ssh_row = row![
        text("SSH").size(10).color(TEXT_MUTED), Space::with_width(6),
    ...
    let action_btn: Element<Message> = if repo.is_cloning {
        ...
        button(text("Clone").size(12).color(GREEN))
            ...
            .style(|_, status| match status {
                ...
                _ => iced::widget::button::Style { background: Some(GREEN_BG.into()), text_color: GREEN,
                    border: Border { radius: 8.0.into(), ..Default::default() }, ..Default::default() },
            }).into()
    };

    container(column![
        name_row, Space::with_height(3),
        text(repo.full_name.as_str()).size(11).color(TEXT_MUTED),
        Space::with_height(6), ssh_row, Space::with_height(14),
...
fn filter_tab<'a>(label: &'a str, filter: ProviderFilter, active: &ProviderFilter) -> Element<'a, Message> {
    let is_active = &filter == active;
    let (color, bg, border) = if is_active { (TEAL, TEAL_BG, TEAL_BORDER) } else { (TEXT_MUTED, BG_SURFACE, BORDER_SUBTLE) };
    button(text(label).size(11).color(color))
        .on_press(Message::Repos(ReposMessage::SetFilter(filter)))
        .padding(Padding::from([5, 12]))
        .style(move |_, status| match status {
            iced::widget::button::Status::Hovered | iced::widget::button::Status::Pressed =>
                iced::widget::button::Style { background: Some(TEAL_BG.into()), text_color: TEAL,
                    border: Border { color: TEAL_BORDER, width: 1.0, radius: 6.0.into() }, ..Default::default() },
            _ => iced::widget::button::Style { background: Some(bg.into()), text_color: color,
                border: Border { color: border, width: 1.0, radius: 6.0.into() }, ..Default::default() },
        }).into()
}
