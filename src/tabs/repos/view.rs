// src/tabs/repos/view.rs — all UI rendering for the Repos tab
...
pub fn render(tab: &ReposTab) -> Element<'_, Message> {
    ...
    let status_bar = container(row![
        ...
        icon_btn(if tab.fetching { "Fetching…" } else { "Fetch Repos" },
            TEAL, TEAL_BG, TEAL_HOVER, TEAL_BORDER,
            if tab.fetching { None } else { Some(Message::REPOS_Fetch) }),
        Space::with_width(8),
        icon_btn("Check SSH",     TEXT_SECONDARY, BG_HOVER, BG_ELEVATED, BORDER_SUBTLE, Some(Message::REPOS_CheckSsh)),
        Space::with_width(8),
        icon_btn("Open Projects", TEXT_SECONDARY, BG_HOVER, BG_ELEVATED, BORDER_SUBTLE, Some(Message::REPOS_OpenRoot)),
    ...
    let filter_bar: Element<Message> = if !tab.remote_repos.is_empty() {
        ...
        .padding(Padding::from([10, 16])).width(Length::Fill).style(surface_style())
        .into()
    } else {
    ...
    let body: Element<Message> = if tab.remote_repos.is_empty() && !tab.fetching {
        ...
        .width(Length::Fill).padding(Padding::from([48, 0])).center_x(Length::Fill)
        .into()
    } else {
        ...
        if filtered.is_empty() {
            container(text("No repos match your filter").size(14).color(TEXT_MUTED))
                .width(Length::Fill).padding(Padding::from([40, 0])).center_x(Length::Fill)
                .into()
        } else {
...
fn empty_state<'a>() -> Element<'a, Message> {
    let github_hint = container(column![
        row![
            text("GitHub").size(12).color(TEAL), Space::with_width(8),
    ...
    let bitbucket_hint = container(column![
        row![
            text("Bitbucket").size(12).color(BLUE), Space::with_width(8),
    ...
    container(column![
        ...
        github_hint, Space::with_height(8), bitbucket_hint,
        Space::with_height(16), note,
    ].spacing(0))
...
fn filter_tab<'a>(label: &'a str, filter: ProviderFilter, active: &ProviderFilter) -> Element<'a, Message> {
    ...
    button(text(label).size(11).color(color))
        .on_press(Message::REPOS_SetFilter(filter))
