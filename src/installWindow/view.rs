...
pub fn view<'a>(
    ...
    let pkg_rows: Vec<Element<Message>> = INSTALL_ITEMS
        ...
        .map(|item| {
            ...
            package_row(item.package, item.purpose, status, dot_color)
        })
        .collect();
    ...
    .height(Length::Fill)
    .style(|_: &iced::Theme| container::Style {
        background: Some(OVERLAY_STRONG.into()),
        ..Default::default()
    })
    .into()
}
...
fn package_row<'a>(
    ...
    container(
        row![
            ...
            Space::with_width(10),
            text(package).size(12).color(TEXT_PRIMARY).width(220),
            text(purpose).size(11).color(TEXT_MUTED).width(Length::Fill),
